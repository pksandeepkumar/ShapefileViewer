...
package com.texus.shapefileviewer.shape.diewald_shapeFile.files.shp;



import com.texus.shapefileviewer.shape.diewald_shapeFile.files.ShapeFileReader;
import com.texus.shapefileviewer.shape.diewald_shapeFile.files.shp.shapeTypes.ShpMultiPoint;
import com.texus.shapefileviewer.shape.diewald_shapeFile.files.shp.shapeTypes.ShpPoint;
import com.texus.shapefileviewer.shape.diewald_shapeFile.files.shp.shapeTypes.ShpPolyLine;
...
public class SHP_File extends ShapeFileReader{
  ...
  public void read() throws Exception{
    ...
    } else if( shape_type.isTypeOfPolygon   ()) { while( bb.position() != bb.capacity()) shapes.add(
            new ShpPolygon(shape_type)   .read(bb) );
    } else if( shape_type.isTypeOfPolyLine  ()) { while( bb.position() != bb.capacity()) shapes.add(
            new ShpPolyLine(shape_type)  .read(bb) );
    } else if( shape_type.isTypeOfPoint     ()) { while( bb.position() != bb.capacity()) shapes.add(
            new ShpPoint(shape_type)     .read(bb) );
    } else if( shape_type.isTypeOfMultiPoint()) { while( bb.position() != bb.capacity()) shapes.add(
            new ShpMultiPoint(shape_type).read(bb) );
